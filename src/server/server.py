...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{str(Path.home())}/.Truflow/logs/server_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.log"
        ),
...
# Load share database
truflow_db = TinyDB(f"{str(Path.home())}/.Truflow/db/db.json")
            ...
            match request["type"]:
                ...
                case HeaderCode.UPDATE_HASH:
                    ...
                    if username is not None:
                        ...
                        try:
                            ...
                            user_share = truflow_db.search(User.uname == username)[0]["share"]
                            ...
                            truflow_db.update({"share": user_share}, User.uname == username)
                        except IndexError:
                            raise RequestException(
                                msg=f"Username does not exist",
                                code=ExceptionCode.NOT_FOUND,
                ...
                case HeaderCode.FILE_BROWSE:
                    ...
                    if username is not None and user_exists:
                        ...
                        browse_files: list[DBData] = truflow_db.search(User.uname == request["query"].decode(FMT))
                ...
                case HeaderCode.FILE_SEARCH:
                    ...
                    if username is not None and search_query:
                        data: list[DBData] = truflow_db.all()
