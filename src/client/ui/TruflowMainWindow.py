...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{str(Path.home())}/.Truflow/logs/client_{datetime.now().strftime('%d-%m-%Y_%H-%M-%S')}.log"
        ),
...
class SaveProgressWorker(QObject):
    ...
    def dump_progress_data(self) -> None:
        ...
        with (Path.home() / ".Truflow/db/transfer_progress.pkl").open(mode="wb") as transfer_progress_dump:
        ...
        with (Path.home() / ".Truflow/db/dir_progress.pkl").open(mode="wb") as dir_progress_dump:
        ...
        with (Path.home() / ".Truflow/db/progress_widgets.pkl").open(mode="wb") as progress_widgets_dump:
                    ...
                    try:
                        ...
                        if message_content:
                            ...
                            if user_settings["show_notifications"] and username != selected_uname:
                                ...
                                notif = Notify()
                                notif.application_name = "Truflow"
                                notif.title = "Message"
...
class Ui_TruflowMainWindow(QWidget):
    """A worker that requests files or directories to be downloaded from the selected peer
    ...
    def __init__(self, MainWindow):
        ...
        super(Ui_TruflowMainWindow, self).__init__()
        try:
            global user_settings
            global dir_progress
            global transfer_progress
            ...
            try:
                ...
                with (Path.home() / ".Truflow/db/transfer_progress.pkl").open(mode="rb") as transfer_progress_dump:
            ...
            try:
                ...
                with (Path.home() / ".Truflow/db/dir_progress.pkl").open(mode="rb") as dir_progress_dump:
        ...
        try:
            with (Path.home() / ".Truflow/db/progress_widgets.pkl").open(mode="rb") as progress_widgets_dump:
    ...
    def retranslateUi(self, MainWindow) -> None:
        ...
        MainWindow.setWindowTitle(QCoreApplication.translate("MainWindow", "Truflow", None))
        self.label_3.setText(QCoreApplication.translate("MainWindow", f"Truflow / {self.user_settings['uname']}", None))
